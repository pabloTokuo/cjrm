...
//   const isRequestReceived = request.readyState === 4 && request.status === 200;
...
person.walk = meters => {
  person.metrosCaminhados += meters;
...
person.introduction = (nome, sobrenome, idade, altura, peso, metrosCaminhados) => {
  const personSexIsFemale = person.sexo === "Feminino" ? "a" : "o";
  const personAgeIs1 = person.idade === 1 ? "ano" : "anos";
  const metersWalkedIs1 = person.metrosCaminhados === 1 ? "metro" : "metros";

  return `Oi. Eu sou ${personSexIsFemale} ${nome} ${sobrenome}, tenho ${idade} ${personAgeIs1}, ${altura} metros de altura, 
  peso ${peso} quilos e, só hoje, eu já caminhei ${metrosCaminhados} 
  ${metersWalkedIs1}.`;
};

console.log(person.introduction(person.nome, person.sobrenome, person.idade, person.altura, person.peso, person.metrosCaminhados));
