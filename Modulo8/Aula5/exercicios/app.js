...
const newArrayOfOrderedNames = names.map(name => name).sort();

console.log(newArrayOfOrderedNames);
...
const newCharactersArray = characters.map(item => item);  

const orderedCharactersId = newCharactersArray.sort((item1, item2) => item1.id - item2.id);

console.log(orderedCharactersId);
...
const newOrderedNumbersArray = numbers.map(number => number).sort((number1, number2) => number1 - number2);

console.log(newOrderedNumbersArray);
...
const newPeopleArray = people.map(name => name).sort().reverse();
...
const newIngredientsArray = ingredients.reduce((acc, ingredient, index, array) => {
  const correctWord = /a$/.test(ingredient) ? "cozida" : "cozido";

  if(index === array.length - 1) {
    return acc + `${ingredient} ${correctWord}`;
  }
...
const amountOfPeopleWhoWatchDisneyMovies = topBrazilmovies.filter(({ distributedBy }) => distributedBy === "Disney").reduce((acc, item) => {
  return acc + item.peopleAmount;
}, 0)

console.log(amountOfPeopleWhoWatchDisneyMovies);
...
// const movieNames = topBrazilmovies.reduce((acc, item) => acc + `<li>${item.title}</li>`, '');
