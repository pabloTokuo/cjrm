...
const orderPeopleByScore = people
  .map((people) => people)
  .sort((item1, item2) => item1.score - item2.score);

console.log(orderPeopleByScore);
...
const animalsQuantityLettersName = animals.map((animal) => animal.length);
...
const newDataWithoutChina = data
  .filter((data) => data.country !== "China")
  .reduce((acc, data) => {
