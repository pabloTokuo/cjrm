...
const covertToString = (value) => {
  return String(value);
}

const text = covertToString('Pablo');
// console.log(text);
...
const lengthString = (value) => {
  const valueLength = value.length;
  return valueLength;
}

// console.log(lengthString('Pablo'));
...
const charString = (a, value) => {  
  let result = value.indexOf(a);
  return result;
}

// console.log(charString('a', 'Pablo'));
...
const booleanFunction = (value) => {
  return socialNetworks.includes(value);
}

// console.log(booleanFunction('twitter'));
...
const arrayConcat = (array1, array2) => {
  const totalArr = array1.concat(array2);
  return totalArr;
}

// console.log(arrayConcat(fruits, socialNetworks));
...
const lastItemRemoved = (item) => {
  return item.pop();
}

// console.log(lastItemRemoved(fruits), fruits);
...
const callbackFunction = (invokeCallBack) => {
  invokeCallBack();
}
...
// callbackFunction(nameFunction('Pablo Tokuo'));
...
review.forEach((paragaph) => {
  paragraphs += `<p>${paragaph}</p>`;
})
...
const allLikes = (peoples = []) => {
  switch(peoples.length) {
    case 0:
      return 'Ninguém curtiu isso'
    case 1:
      return `${peoples[0]} curtiu isso`
    case 2:
      return `${peoples[0]} e ${peoples[1]} curtiram isso`
    case 3:
      return `${peoples[0]}, ${peoples[1]} e ${peoples[2]} curtiram isso`
    default:
      return `${peoples[0]}, ${peoples[1]} e mais ${peoples.length - 2} curtiram isso`
  }
}

console.log(allLikes(['Pablo', 'Pablo', 'Pablo', 'Pablo', 'Pablo']));
